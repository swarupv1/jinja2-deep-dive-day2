...
## Task 1 - Building initial stages of an Ansible module

Ansible modules are the building blocks of Ansible playbooks. The modules are used inside tasks within a playbook and do all the automation work based on the purpose of the module. These modules are small pieces of **Python** code that are triggered from the **YAML** syntax used in a playbook.

Ansible already comes installed with a good number of modules to cover most of the automation tasks out there, but sometimes there are going to be certain tasks that some modules might not fully support.

### Step 1

In the terminal of the workstation, change or create if not present to `/home/ntc/labs/lab05` folder.
...
Create a folder named `modules` and an empty file called `iosxe_gather_configs.py`.
...
Open up the `iosxe_gather_configs.py` script with an editor of choice to start editing. Inside the file add `#!/usr/bin/python3` on the top-most line and on the next line import the `AnsibleModule` from the `ansible.module_utils.basic` library.

The `AnsibleModule` will help handle incoming parameters and exiting the program using the `module.exit_json()` method. 
...
After importing the `AnsibleModule` class object build a `main()` function and create a variable called `module` to store the `AnsibleModule` object. Inside the `AnsibleModule` object pass in `argument_spec` as an argument with an empty dictionary for now. 
...
Create a variable called `response` with a value containing a dictionary of `{"host": "csr1"}`.
...
The `AnsibleModule` will help handle incoming parameters and exiting the program using the `module.exit_json()` method at the end of the script. The `module.exit_json()` method will take in two arguments `changed=False` and `meta=response`.
...
At the end of the script add the entry point to the script.
...
Create a `YAML` file in the `lab05` folder called `test_module.yml`. 
...
In this task the focus will be in implementing the code that will make an API call against IOSXE devices to `GET` configuration data.
...
Inside the `def main()` function add the `url`, `auth`, `headers` and modify the `response` variable.
...
Change back to the `lab05` folder and execute the `test_module.yml` playbook again to see the output.
...
- name: TEST CUSTOM MODULE WORKS
  ...
  tasks:
    ...
    - name: VIEW THE DATA STORED IN RESULT
      debug: 
        var: result['meta']['Cisco-IOS-XE-native:native'].keys()
```
...
Modify the playbook so it can only access the device version.
...
- name: TEST CUSTOM MODULE WORKS
  ...
  tasks:
    ...
    - name: VIEW THE DATA STORED IN RESULT
      debug: 
        var: result['meta']['Cisco-IOS-XE-native:native']['version']
```
...
TASK [VIEW THE DATA STORED IN RESULT] *******************************************************************************************************
ok: [localhost] => {
    "result['meta']['Cisco-IOS-XE-native:native']['version']": "17.1"
}
...
## Task 4 Optimizing module with parameters
...
Modify each key inside `module_args` to specify what data type to expect from the user and if the parameter is required or not. Since these parameters will be needed to communicate with the networking devices then they will need to be set as required. 
...
Modify the `test_module.yml` playbook to take in all `iosxe` devices and paramatize the data that will be taken as input to the module. 
...
After making and saving the changes execute the playbook with the inventory file to view the results.
...
Currently the module can only return the entire device configuration a `filter` can be created as a parameter so that the user can specify as a value to only return certain data rather than the entire configuration. Modify the `iosxe_gather_configs.py` script so it can filter out the device configuration.
...
> Note: Try other choices and view the results.
